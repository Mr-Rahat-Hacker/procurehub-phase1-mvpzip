          ...
          <tbody>
            {loading ? (
              ...
              <tr key={v.id}>
                ...
                <td style={{ textTransform: 'capitalize' }}>{v.category?.replace(/_/g, ' ')}</td>
                <td>
                ...
                <td>
                  <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                    <Link to={`/vendors/${v.id}`} className="btn btn-ghost" style={{ padding: '4px 8px', fontSize: '12px' }}>View</Link>
                    <button className="btn btn-ghost" style={{ padding: '4px 8px', fontSize: '12px' }}
                      onClick={() => { setEditVendor(v); setShowModal(true) }}>Edit</button>
                    {isManager && (v.status === 'pending' || v.status === 'under_review') && (
                      <>
                        <button className="btn btn-sm" style={{ background: 'var(--green-dim)', color: 'var(--green)', border: '1px solid rgba(34,197,94,0.2)', padding: '4px 8px' }}
